...
def summarize_hours_by_type_per_area(df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    ...
    for _, r in pivot.iterrows():
        area = r["__CraftDesc"]
        t = r["__TypeDesc"]
        h = float(r["__H"] or 0.0)
        result.setdefault(area, {})
        result[area][t] = result[area].get(t, 0.0) + h
    return result

def render_pie_pages(summary: Dict[str, Dict[str, float]], selected_date: str, label_mode: str) -> List[bytes]:
    """Return a list of PNG bytes, each a page of pies (3x2 grid) for areas sorted by total hours desc.
       label_mode: 'percent' or 'hours' for wedge labels.
    """
    ...
    for i in range(0, len(items), pies_per_page):
        ...
        for ax, (area, typemap) in zip(axs, chunk):
            ...
            if sum(sizes) <= 0:
                ...
                continue
            # consolidate tiny slices for readability (<=2%)
            total0 = sum(sizes)
            lbl2, sz2 = [], []
            other = 0.0
            for l, s in zip(labels, sizes):
                if total0 > 0 and (s/total0) < 0.02:
                    other += s
                else:
                    lbl2.append(l); sz2.append(s)
            if other > 0:
                lbl2.append("Other"); sz2.append(other)
            total = sum(sz2)

            if label_mode == "percent":
                autopct = "%1.0f%%"
            else:
                def _autopct(pct, _total=total):
                    val = _total * pct / 100.0
                    return f"{val:.1f}h"
                autopct = _autopct

            wedges, texts, autotexts = ax.pie(sz2, autopct=autopct, startangle=90, counterclock=False)
            ax.set_title(f"{area} — {total:.2f} h", fontsize=10)
            if len(lbl2) <= 6:
                ax.legend(wedges, lbl2, fontsize=7, loc="lower center", bbox_to_anchor=(0.5, -0.15), ncol=2)
        buf = io.BytesIO()
...
# ------------------------ PDF Output ------------------------
def make_pdf(selected_date: str, crafts: Dict[str, List[Dict[str, Any]]], cover_summary: Dict[str, Dict[str, float]], label_mode: str) -> bytes:
    if PDF_ENGINE == "reportlab":
        ...
        for idx, png in enumerate(pie_pages):
            ...
            if idx == 0:
                unit = "%" if label_mode == "percent" else "hours"
                story += [Paragraph(f"Daily Report — {selected_date}", title_style), Spacer(1, 6),
                          Paragraph(f"Summary: Hours by Work Order Type per Area ({unit})", styles["Normal"]), Spacer(1, 6)]
            story.append(img)
    ...
    for idx, png in enumerate(pie_pages):
        ...
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
            ...
            img_path = tmp.name
        pdf.image(img_path, x=margin, y=margin, w=pdf.w - margin*2)
...
# ------------------------ UI ------------------------
st.title("Craft-Based Daily Report (Excel → PDF) — Cover Page with Pie Charts")
...
with st.sidebar:
    ...
    st.markdown("5) Review and **Download PDF**. The first page(s) show pie charts by area; details follow.")
...
if df is not None and addr_map is not None and selected_date:
    ...
    st.subheader("Summary — Hours by Work Order Type per Area")
    st.caption("This summary is also rendered as the first page(s) in the PDF.")
    ...
    pdf_bytes = make_pdf(selected_date, crafts, cover_summary, label_mode)
