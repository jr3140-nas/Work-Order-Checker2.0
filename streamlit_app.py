# streamlit_app.py — v10e (pie cover fix + preview)
import io, re, math
...
def make_pdf(selected_date: str, crafts: Dict[str, List[Dict[str, Any]]], cover_summary: Dict[str, Dict[str, float]], label_mode: str, cap_desc=600, cap_prob=600) -> bytes:
    if PDF_ENGINE == "reportlab":
        ...
        story: List = []
        # Cover pages (use ImageReader to ensure PNGs embed)
...
# ------------------------ UI ------------------------
st.title("Craft-Based Daily Report (Excel → PDF) — v10e")
...
if df is not None and addr_map is not None and selected_date:
    ...
    with st.expander("Preview cover pies (for verification)", expanded=False):
        pie_imgs = render_pie_pages(cover_summary, selected_date, label_mode)
        for i, img in enumerate(pie_imgs, start=1):
            st.image(img, caption=f"Cover Page {i} preview", use_container_width=True)
