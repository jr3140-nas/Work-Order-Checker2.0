...
def area_passes(desc: str, selected: List[str]) -> bool:
    """Return True if desc falls into one of the selected groups. Empty -> include all."""
    if not selected:
        return True
    return craft_category(desc) in set(selected)

# ------------------------ Report Logic ------------------------
def build_report(df: pd.DataFrame, selected_date: str, name_to_craft: Dict[str, str], selected_groups: List[str]):
    df = df.copy()
    ...
    df = df[df["__CraftDesc"].map(lambda x: area_passes(x, selected_groups))]
...
with st.sidebar:
    ...
    st.markdown("5) Use **Group Filter** to focus on Mechanical, Electrical, or HVAC & Utilities.")
...
area_mode = st.multiselect("Group Filter", options=["Electrical","Mechanical","Turns","Segment Shop","Utilities"], default=["Electrical","Mechanical","Turns","Segment Shop","Utilities"])
