...
def _soft_wrap_for_pdf(s: str, max_chars: int = 600, unbroken_chunk: int = 40) -> str:
    """
    Avoid single-row overflow in ReportLab tables:
    - insert spaces inside long unbroken tokens so Paragraph can wrap
    - cap total characters per cell
    """
    if s is None: return ""
    s = str(s)
    if not s: return ""
...
# ------------------------ UI ------------------------
st.title("Craft-Based Daily Report (Excel → PDF) — Cover Page & Safe PDF Tables")
