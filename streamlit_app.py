# streamlit_app.py — v10 (stable snapshot)
import io, re, math
...
def summarize_hours_by_type_per_area(df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    tmp = df.copy(); tmp["__TypeDesc"] = tmp["Type"].map(lambda x: type_to_desc(x) or "Unknown Type")
...
def make_pdf(selected_date: str, crafts: Dict[str, List[Dict[str, Any]]], cover_summary: Dict[str, Dict[str, float]], label_mode: str, cap_desc=600, cap_prob=600) -> bytes:
    if PDF_ENGINE == "reportlab":
        ...
        for idx, png in enumerate(pie_pages):
            img = RLImage(io.BytesIO(png)); iw, ih = getattr(img, 'imageWidth', None), getattr(img, 'imageHeight', None)
            if iw is None or ih is None: iw, ih = img.wrap(0, 0)
            max_w, max_h = doc.width, doc.height; scale = min(max_w/iw, max_h/ih)*0.95
            img.drawWidth = iw*scale; img.drawHeight = ih*scale; story.append(img)
            if idx < len(pie_pages)-1: story.append(PageBreak())
        # Details
        story += [Paragraph(f"Daily Report — {selected_date}", title_style), Spacer(1, 6), Paragraph("Sorted by Work Order # within each craft", styles["Normal"]), Spacer(1, 12)]
        ...
        for craft, rows in crafts.items():
            ...
            for r in rows:
                matrix.append([str(r.get("Name","")), str(r.get("Work Order #","")), f'{float(r.get("Sum of Hours",0)):.2f}', str(r.get("Type","")), str(r.get("Description","")), str(r.get("Problem",""))])
            ...
            for raw in matrix[1:]:
                ...
                data.append([Paragraph(name, body8), Paragraph(wo, body8), Paragraph(hrs, body8), Paragraph(typ, body8), Paragraph(desc, body8), Paragraph(prob, body8)])
            tbl = Table(data, repeatRows=1, colWidths=col_widths); tbl.setStyle(table_style); story.append(tbl); story.append(Spacer(1,10))
    ...
    pdf.add_page(); pdf.set_left_margin(margin); pdf.set_right_margin(margin); pdf.set_auto_page_break(auto=True, margin=margin)
    ...
    def compute_fpdf_widths(rows: List[List[str]]) -> List[float]:
        ...
        total=sum(naturals)
        if total<=page_inner_width: return naturals
        over=total-page_inner_width; shrinkable=[max(0.0,naturals[i]-minw[i]) for i in range(len(naturals))]; total_shrink=sum(shrinkable)
        if total_shrink<=0:
            scale=page_inner_width/total if total>0 else 1.0; return [w*scale for w in naturals]
        widths=[]; 
        for i,w in enumerate(naturals):
            reduce=over*(shrinkable[i]/total_shrink) if total_shrink>0 else 0.0
            widths.append(max(minw[i], w-reduce))
        if sum(widths)>page_inner_width:
            scale=(page_inner_width-0.01*page_inner_width)/sum(widths); widths=[w*scale for w in widths]
        return widths
    ...
    for craft, rows in crafts.items():
        ...
        pdf.ln(th); pdf.set_font("Helvetica","",8)
        for raw in matrix[1:]:
...
st.title("Craft-Based Daily Report (Excel → PDF) — v10")
