...
# --- Helper: render non-scrollable tables that fit the page ---
import pandas as _pd
_st_table = st.table  # alias original Streamlit table to avoid any regex replacements

def render_table(_df):
    """Render a full, non-scrollable table. Avoid recursion by calling the alias."""
    if isinstance(_df, _pd.io.formats.style.Styler):
        try:
            _df = _df.data  # fall back to underlying DataFrame if a Styler is passed
        except Exception:
            pass
    _st_table(_df)
...
def render_table(_df):
    if isinstance(_df, _pd.DataFrame):
        # wrap long text to avoid horizontal scrollbars
        styler = _df.style.set_table_styles([
            dict(selector='th', props=[('white-space','normal')]),
            dict(selector='td', props=[('white-space','normal'), ('word-wrap','break-word')]),
        ])
        render_table(styler)
    else:
        render_table(_df)
...
if df is not None and addr_map is not None and selected_date:
    ...
    for craft, rows in crafts.items():
        ...
        render_table(pd.DataFrame(rows, columns=["Name","Work Order #","Sum of Hours","Type","Description","Problem"]))
