# streamlit_app.py — v10g (UI cleanup, robust pies)
import io, re, math
...
OTHER_COLOR = "#b0b0b0"
import numpy as np
PALETTE = [matplotlib.colors.to_hex(c) for c in plt.get_cmap("tab20").colors]
...
def build_global_color_map(summary: Dict[str, Dict[str, float]]) -> Dict[str, str]:
    ...
    # deterministic palette across sessions
    for i, lab in enumerate(sorted(labels, key=str.lower)):
...
def make_pdf(selected_date: str, crafts: Dict[str, List[Dict[str, Any]]], cover_summary: Dict[str, Dict[str, float]], label_mode: str, cap_desc=600, cap_prob=600) -> bytes:
    if PDF_ENGINE == "reportlab":
        ...
        for idx, png in enumerate(pie_pages):
            data = bytes(png)
            bio1 = io.BytesIO(data); bio2 = io.BytesIO(data)
            iw, ih = ImageReader(bio1).getSize()
            max_w, max_h = doc.width, doc.height
            scale = min(max_w/iw, max_h/ih) * 0.95
            story.append(RLImage(bio2, width=iw*scale, height=ih*scale))
...
def main():
    ...
    st.title("Craft-Based Daily Report (Excel → PDF) — v10g")
    with st.sidebar:
    ...
    if df is not None and addr_map is not None and selected_date:
        ...
        # Draw tables per craft
        for craft, rows in crafts.items():
            st.subheader(craft)
            st.dataframe(pd.DataFrame(rows, columns=["Name","Work Order #","Sum of Hours","Type","Description","Problem"]), use_container_width=True)
